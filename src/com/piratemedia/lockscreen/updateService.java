...
public class updateService extends Service {
	...
	public void onCreate() {
		...
		if (!utils.getCheckBoxPref(getBaseContext(),
				LockscreenSettings.ENABLE_KEY, true)) {
			stopSelf();
		}

		mNM = (NotificationManager) getSystemService(NOTIFICATION_SERVICE);
		// Start as foreground if user settings say so
		foregroundStuff(utils.getCheckBoxPref(this,
				LockscreenSettings.SERVICE_FOREGROUND, true));
		// SCREEN ON/OFF suff
		if (mReceiver == null) {
			...
			mReceiver = new BroadcastReceiver() {
				...
				public void onReceive(Context context, Intent intent) {
					...
					if (action.equals(Intent.ACTION_SCREEN_ON)) {
						...
						if (!inCall()) {
							Intent lock = utils
									.getLockIntent(updateService.this);
	...
	private void foregroundStuff(boolean foreground) {
		if (foreground) {
			...
			Notification notification = new Notification(
					R.drawable.status_icon, text, System.currentTimeMillis());
	...
	public int onStartCommand(Intent aIntent, int aFlags, int aStartId) {
		...
		// Cos i like 2! its all lie 3 isnt the magic number! 2 is!!!!!! AWWWWWW
		// YEAAHHHHH MOTHER FUKA HA HA
	}

	@Override
	public void onStart(Intent aIntent, int aStartId) {
		// This thing is needed when the service is restarted by the system,
		// cause aIntent can be null
		...
		if (action.equals("com.android.music.playbackcomplete")
			...
			// The song has ended
				...
				|| action.equals("com.android.music.queuechanged") || action
			...
			bindService(new Intent().setClassName("com.android.music",
					...
					new ServiceConnection() {
						...
						public void onServiceConnected(ComponentName aName,
								IBinder aService) {
							com.android.music.IMediaPlaybackService mService = com.android.music.IMediaPlaybackService.Stub
									.asInterface(aService);
		...
		} else if (action.equals("com.htc.music.playbackcomplete")
				&& getPlayer() == 2) {
			// The song has ended
		} else if ((action.equals("com.htc.music.playstatechanged")
				|| action.equals("com.htc.music.metachanged")
				|| action.equals("com.htc.music.queuechanged") || action
				.equals("com.htc.music.playbackcomplete")) && getPlayer() == 2) {

			bindService(new Intent().setClassName("com.htc.music",
					"com.htc.music.MediaPlaybackService"),
					new ServiceConnection() {

						public void onServiceConnected(ComponentName aName,
								IBinder aService) {
							com.htc.music.IMediaPlaybackService mService = com.htc.music.IMediaPlaybackService.Stub
									.asInterface(aService);
		...
		} else if (action.equals("com.piratemedia.musicmod.playbackcomplete")
				&& getPlayer() == 3) {
			// The song has ended
		} else if ((action.equals("com.piratemedia.musicmod.playstatechanged")
				|| action.equals("com.piratemedia.musicmod.metachanged")
				|| action.equals("com.piratemedia.musicmod.queuechanged") || action
				.equals("com.piratemedia.musicmod.playbackcomplete"))
				&& getPlayer() == 3) {

			bindService(new Intent().setClassName("com.piratemedia.musicmod",
					"com.piratemedia.musicmod.MediaPlaybackService"),
					new ServiceConnection() {

						public void onServiceConnected(ComponentName aName,
								IBinder aService) {
							com.piratemedia.musicmod.IMediaPlaybackService mService = com.piratemedia.musicmod.IMediaPlaybackService.Stub
									.asInterface(aService);
			...
			// The song has ended
				...
				|| action.equals("com.tbig.playerpro.queuechanged") || action
				.equals("com.tbig.playerpro.playbackcomplete"))
				&& getPlayer() == 4) {

			bindService(new Intent().setClassName("com.tbig.playerpro",
					"com.tbig.playerpro.MediaPlaybackService"),
					new ServiceConnection() {

						public void onServiceConnected(ComponentName aName,
								IBinder aService) {
							com.tbig.playerpro.IMediaPlaybackService mService = com.tbig.playerpro.IMediaPlaybackService.Stub
									.asInterface(aService);
				...
				.equals("org.abrantix.rockon.rockonnggl.playbackcomplete")
				&& getPlayer() == 4) {
			// The song has ended
		} else if ((action
				.equals("org.abrantix.rockon.rockonnggl.playstatechanged")
				|| action.equals("org.abrantix.rockon.rockonnggl.metachanged")
				|| action.equals("org.abrantix.rockon.rockonnggl.queuechanged") || action
				.equals("org.abrantix.rockon.rockonnggl.playbackcomplete"))
				&& getPlayer() == 5) {

			bindService(new Intent().setClassName(
					"org.abrantix.rockon.rockonnggl",
					"org.abrantix.rockon.rockonnggl.RockOnNextGenService"),
					new ServiceConnection() {

						public void onServiceConnected(ComponentName aName,
								IBinder aService) {
							org.abrantix.rockon.rockonnggl.IRockOnNextGenService mService = org.abrantix.rockon.rockonnggl.IRockOnNextGenService.Stub
									.asInterface(aService);
			...
			foregroundStuff(utils.getCheckBoxPref(getBaseContext(),
					LockscreenSettings.SERVICE_FOREGROUND, true));
	...
	public int getPlayer() {
		String playerString = utils.getStringPref(this,
				LockscreenSettings.KEY_MUSIC_PLAYER, "1");
		int player = Integer.parseInt(playerString);
		return player;
	}

	private boolean inCall() {
		TelephonyManager telMan = ((TelephonyManager) getBaseContext()
